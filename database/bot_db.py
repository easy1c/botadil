...
class Database:
    ...
    def sql_create_tables(self):
        if self.connection:
         ...
         self.connection.execute(sql_queries.CREATE_PROFILE_USER_TABLE_QUERY)
    ...
    def sql_insert_ban_user(self, tg_id):
        existing_record = self.sql_select_ban_user(tg_id)
        if existing_record:
            self.sql_update_ban_count(tg_id)
        else:
            self.cursor.execute(
    ...
    def sql_insert_profile(self, tg_id, nickname, bio, age, sing, photo):
        self.cursor.execute(
            sql_queries.INSERT_PROFILE_QUERY,
            (None, tg_id, nickname,bio, age, sing, photo )
        )
        self.connection.commit()
